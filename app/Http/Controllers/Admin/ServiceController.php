...
class ServiceController extends Controller
{
	...
	{
        $type = $request->type ?? 1;
        if ($type == ServiceType::TRAI_NGHIEM) {
            $title = 'trải nghiệm';
        } elseif ($type == ServiceType::UU_DAI) {
            $title = 'ưu đãi';
        } elseif ($type == ServiceType::GOI_NGHI_DUONG) {
	...
	public function create(Request $request)
	{
		...
		return view($this->view.'.create' , compact('type', 'title'));
	}
	...
	public function store(Request $request)
	{
        $rules = [
            ...
            'content' => 'required',
            // 'base_price' => 'required_if:type,3',
            'price' => 'nullable|integer|max:'.$request->base_price,
            'image' => 'required|file|mimes:jpg,jpeg,png|max:2000'
        ];
        if ($request->type == ServiceType::GOI_NGHI_DUONG) {
            $rules['base_price'] = 'required_if:type,3|integer';
        }
	...
	public function edit(Request $request,$id)
	{
        ...
        $type = $object->serviceType->type ?? 1;
        if ($type == ServiceType::TRAI_NGHIEM) {
            $title = 'trải nghiệm';
        } elseif ($type == ServiceType::UU_DAI) {
            $title = 'ưu đãi';
		...
		return view($this->view.'.edit', compact('object', 'type', 'title'));
	}
